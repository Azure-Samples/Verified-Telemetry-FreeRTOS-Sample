...
extern "C"
{
    ...
    static mcp320x_err_t _mcp320x_sample_raw(mcp320x_handle_t handle,
                                             ...
                                             float *value, spi_transaction_t* transaction);
                                   ...
                                   float *value, spi_transaction_t* transaction)
    {
        ...
        return _mcp320x_sample_raw(handle, channel, read_mode, 1, value, transaction);
    }
    ...
    //     return _mcp320x_sample_raw(handle, channel, read_mode, sample_count, value, transaction);
    // }
                                             ...
                                             float *value, spi_transaction_t* transaction)
    {
        ...
        for (uint16_t i = 0; i < sample_count; i++)
        {
            ...
            sum +=  (uint16_t)((transaction->rx_data[1] & 15) << 8) |  (uint16_t)transaction->rx_data[2];
        }
